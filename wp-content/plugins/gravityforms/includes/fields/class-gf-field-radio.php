	...
	public function get_radio_choices( $value = '', $disabled_text, $form_id = 0 ) {
		...
		if ( is_array( $this->choices ) ) {
			...
			foreach ( $this->choices as $choice ) {
				...
				$choices .= sprintf( "<li class='gchoice_$id'><input name='input_%d' type='radio' value='%s' %s id='choice_%s' $tabindex %s $logic_event %s />%s %s</li>", $this->id, esc_attr( $field_value ), $checked, $id, $disabled_text, $input_focus, $label, apply_filters( 'gform_after_field', '', $this->id, $this->id, $field_value ) );
